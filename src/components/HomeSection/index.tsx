...
import projects from "../../public/header-bg.svg";
...
export const HomeSection = (): JSX.Element => {
  ...
  const splitText = (text: string, className: string = "") => text.split("").map((char, index) => (
    <motion.span
      key={index}
      className={className}
  ...
  return (
    <section
      id="inicio"
      className="z-0 flex w-screen flex-col items-center justify-center border border-b-[0.15rem] border-purple-500 pb-16 lg:flex-row"
    >
      <section className="xl:w-9/12 flex flex-col items-center justify-center border-4 border-blue-600">
        <div className="w-11/12 xl:w-full gap-2 self-center border border-yellow-300 lg:w-9/12">
          ...
          <motion.h1
            className="inline-block text-3xl font-bold leading-normal tracking-normal md:text-6xl"
            ref={ref}
            initial="hidden"
      ...
      <div className={`border border-orange-500 hidden lg:flex img-projects-bg`}>
        {/* <Image
        className="hidden lg:flex"
        src={projects}
        alt="Projetos anteriores" 
        width={500} 
        height={600}
        /> */}
      </div>
